...
function Genre(name) {
    ...
    this.getData = function() {
        let firstAndLastLetter = (this.name[0] + this.name[this.name.length -1]).toUpperCase();
...
function Movie(title, genre, length) {
    ...
    this.length = length;
    this.getData = function() {
        return `${this.title}, ${this.length } min, ${this.genre.getData()}` //this.title + ', ' + this.length + ' min, ' + this.genre.getData();
    }
}
//// PROGRAM ////
function Program(date) {
    ...
    this.getData = function() {
        let lengthOfMovies = this.listOfMovies.map(function(el){
            return el.length;
        });
    ...
    ${elem.getData()}`//'\n\t\t' + elem.getData();
        ...
        let nekaglupost = `${this.date}, program length from all movies ${total} ${list}` //'\t' + this.date + ', program length from all movies ' + total + list;
        return nekaglupost;
    }
}
///// FESTIVAL /////
